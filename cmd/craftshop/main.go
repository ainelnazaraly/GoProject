...
func main() {
	...
	flag.StringVar(&cfg.db.dsn, "db-dsn", "postgres://postgres:1234@localhost/CraftShop?sslmode=disable", "PostgreSQL DSN")
	flag.Parse()
...
func (app *application) run() {
	...
	// Retrieve a seller by sellerId
	v1.HandleFunc("/sellers/{sellerId:[0-9]+}", app.getSellerHandler).Methods("GET")
	// Update a seller's information by sellerId
	v1.HandleFunc("/sellers/{sellerId:[0-9]+}", app.updateSellerHandler).Methods("PUT")
	// Delete a seller by sellerId
	v1.HandleFunc("/sellers/{sellerId:[0-9]+}", app.deleteSellerHandler).Methods("DELETE")
